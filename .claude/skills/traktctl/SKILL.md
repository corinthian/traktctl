...
Same shape (show adds episode-aware verbs). Discovery lists: `trending`, `popular`, `anticipated`, `favorited`, `played`, `watched`, `collected`, `streaming`, `updates` (movie also `boxoffice`). Single title: `get`, `aliases`, `translations`, `people` (cast/crew), `ratings`, `related`, `stats`, `studios`, `videos`, `comments`, `lists`, `sentiments`, `watching` (movie `releases`; show `certifications`). Show-only: `progress watched` / `progress collection` (how far through a show — `progress` is a group, the bare verb fails `BAD_REQUEST`), `next-episode` (what's next to watch — empty if none), `last-episode`. "What's <show> about" → `show get`. "What's next for <show>" → `show next-episode`.
...
**"Seen" = watched OR rated — union both, never collection.** The watched history is incomplete: a title can be rated (proof it was seen) without a watched entry, and vice-versa, so a watched-only check gives false "unseen" results. Collection is an ownership record, not a viewing record — a collection-only match is UNSEEN and is never labelled a rewatch. To test seen-status, pull `sync watched get --all` and `sync ratings get --all` for the type (a round 100 or 1000 row count means you got page one, not the library), union them, and match by title+year. **Any question that needs the whole account — every watched title, every rating, a genre tally, a backup — is `sync export`, not a fan of `--all` gets.** It pulls all six kinds past the page cap in one call and cannot come back a page short; `--all` on a single `get` still stops at 100 pages. The `recommend` endpoint's own `--ignore-*` exclusions let partially-watched shows through; cross-check its output the same way.
...
If the user asks for one of these, say it's not in traktctl yet (don't construct a command that will fail): `checkin`, `cert`, `comment`, `media`, `note`, `scrobble`, and the `country`/`genre`/`language`/`network` lookups. E.g. "Trakt has the data but traktctl doesn't expose scrobble/checkin yet — it's a planned v2 group."
